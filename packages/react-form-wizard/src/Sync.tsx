...
export function Sync(props: {
  ...
  useEffect(() => {
    ...
    for (const resource of resources) {
      if (
        (props.targetKind === undefined && resource.kind !== props.kind) ||
        resource.kind === props.targetKind
      ) {
  ...
  useEffect(() => {
    if (Array.isArray(resources)) {
      const resource = resources?.find(
        (resource) => resource.kind === props.kind
      );
      if (resource) {
...
export function SyncCount(props: {
  ...
  useEffect(() => {
    ...
    const targets = resources.filter(
      (resource) => resource.kind === props.targetKind
    );
