...
const StepSetHasValidationErrorContext = createContext<
  (id: string, hasError: boolean) => void
>(() => null);
StepSetHasValidationErrorContext.displayName =
  "StepSetHasValidationErrorContext";
export const useSetStepHasValidationError = () =>
  useContext(StepSetHasValidationErrorContext);
...
export const useStepHasValidationError = () =>
  useContext(StepHasValidationErrorContext);
...
export function StepValidationProvider(props: { children: ReactNode }) {
  ...
  const [hasStepValidationErrors, setHasStepValidationErrorsState] = useState<
    Record<string, true>
  >({});
  const [setHasValidationErrors, setHasValidationErrorsFunction] = useState<
    () => void
  >(() => () => null);
