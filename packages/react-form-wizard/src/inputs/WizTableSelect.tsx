...
export function WizTableSelect<T = any>(props: WizTableSelectProps<T>) {
    ...
    selectedItems = values
      .map((value) =>
        props.items.find((item) =>
          props.valueMatchesItem ? props.valueMatchesItem(value, item) : false
        )
      )
    ...
    (item: T, select: boolean) => {
      if (select) {
        if (!selectedItems.includes(item)) {
          setValue([
            ...(props.itemToValue
              ? selectedItems.map(props.itemToValue)
              : selectedItems),
        ...
        if (props.itemToValue) {
          setValue(
            selectedItems.filter((i) => i !== item).map(props.itemToValue)
          );
        } else {
  ...
  const selectAll = useCallback(
    () =>
      setValue(
        props.itemToValue ? props.items.map(props.itemToValue) : props.items
      ),
  ...
  if (props.items.length === 0) {
    return (
      <ErrorState
        titleText={props.emptyTitle}
        bodyText={props.emptyMessage}
        headingLevel="h4"
      />
    );
  }
...
function BulkSelect(props: {
  ...
  const { selected, selectNoItems, selectPageItems, selectAllItems } =
    useStringContext();
        ...
        <MenuToggle
          ...
          splitButtonItems={[
            <MenuToggleCheckbox
              ...
              key="split-checkbox"
              aria-label={
                anySelected ? deselectAllAriaLabel : selectAllAriaLabel
              }
              isChecked={isChecked}
              onChange={onCheckbox}
            >
              {props.selectedCount !== 0 && (
                <Fragment>{selected(props.selectedCount)}</Fragment>
              )}
            </MenuToggleCheckbox>,
          ]}
        />
