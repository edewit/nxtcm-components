...
export function YamlEditor(props: {
  ...
  useResizeObserver(smallRef, (entry) =>
    setTextWidth(entry.contentRect.width + 48)
  );
            ...
            <small ref={smallRef}>
              {yaml.split("\n").map((line, index) => {
                const backgroundColor =
                  index === errorLine ? "#F203" : undefined;
                if (line === "---") {
                  return (
                    <div
                      key={index}
                      style={{ color: color.divider, backgroundColor }}
                    >
                      {line}
                    </div>
                  );
                ...
                if (parts.length === 1) {
                  return (
                    <div
                      key={index}
                      style={{ color: color.variable, backgroundColor }}
                    >
                      {parts[0]}
                    </div>
                  );
